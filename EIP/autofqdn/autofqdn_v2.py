...
def main():
    ...
    node_url = f"{sds_host.rstrip('/')}/api/v2.0/app/node/add"
    ...
    for ip in ip_list:
        node_payload = {
            ...
            "pool_name": pool_name,
            "node_hostaddr": ip,
            "node_name": ip, # Using the IP address as the node name
            "admin_status": 1,
            "healthcheck_name": "tcp",
            "healthcheck_freq": "10",
            "healthcheck_failover": "3",
            "healthcheck_failback": "3",
            "healthcheck_timeout": "3",
            "healthcheck_params": "443&",
            "warnings": "accept"
        }
        
        try:
            node_response = requests.post(node_url, headers=headers, json=node_payload, verify=False)
            ...
            if not node_data.get("success"):
                print(f"[ERROR] Node creation returned a failure in the payload for {ip}.")
                print(f"[ERROR] Details: {node_data.get('messages')}")
                sys.exit(1)
                
            print(f"[INFO] Node '{ip}' successfully added to pool!")
            
        except requests.exceptions.RequestException as e:
            print(f"[ERROR] HTTP Request failed during node creation for {ip}: {e}")
            sys.exit(1)
        except json.JSONDecodeError:
            print(f"[ERROR] Failed to parse JSON response during node creation for {ip}.")
            print(f"Raw Output: {node_response.text}")
            sys.exit(1)
